...
export async function getDigestList(added){
    return await cadiSqlQuery({
        sql: `SELECT p.nom as last_name, p.prenom as first_name, p.sciper, p.email, p.username as gaspar, b.telephone1 as phone_number 
                FROM Personnes p
                LEFT JOIN Bottin b ON p.sciper = b.sciper  where p.username IS NOT NULL limit 30000`,
        timeout: 50000,
